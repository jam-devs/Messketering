...
export class EquipmentService {
  private readonly equipment$ = new BehaviorSubject<EquipmentItem[]>(SEED);
  ...
  add(item: Omit<EquipmentItem, 'id' | 'createdAt' | 'updatedAt' | 'availableQuantity'>): EquipmentItem {
    ...
    this.equipment$.next([...this.equipment$.value, created]);
    return created;
  }

  update(id: string, patch: Partial<EquipmentItem>): void {
    this.equipment$.next(
      this.equipment$.value.map((e) =>
        e.id === id ? { ...e, ...patch, updatedAt: new Date() } : e
      )
    );
  }
