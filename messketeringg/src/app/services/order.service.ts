...
export class OrderService {
  private readonly orders$ = new BehaviorSubject<Order[]>(SEED);

  constructor(
    private equipmentService: EquipmentService,
    private packageService: PackageService
  ) {
    this.equipmentService.syncAvailability(this.orders$.value);
  }
  ...
  updateKitchenStatus(id: string, kitchenStatus: KitchenStatus): void {
    const patch: Partial<Order> = { kitchenStatus, updatedAt: new Date() };
    const order = this.orders$.value.find((o) => o.id === id);
    if (kitchenStatus === 'ready_for_transport' && order?.logisticsStatus === 'not_started') {
      patch.logisticsStatus = 'preparing_for_delivery';
    }
    if (kitchenStatus === 'completed') patch.orderStatus = 'completed';
    this.patch(id, patch);
  }

  updateLogisticsStatus(id: string, logisticsStatus: LogisticsStatus): void {
    this.patch(id, { logisticsStatus, updatedAt: new Date() });
  }

  updateOrderStatus(id: string, orderStatus: ManualOrderStatus): void {
    this.patch(id, {
      orderStatus,
      kitchenStatus: this.mapOrderToKitchen(orderStatus),
      updatedAt: new Date(),
    });
  }
