...
export class FoodMenuService {
  private readonly items$ = new BehaviorSubject<FoodMenuItem[]>([]);
  ...
  private refresh() {
    this.api.get<FoodMenuItem>('food-menu').subscribe({
      next: (items) => {
  ...
  add(item: Omit<FoodMenuItem, 'id' | 'createdAt' | 'updatedAt'>): FoodMenuItem {
    ...
    this.api.create<FoodMenuItem>('food-menu', {
      ...created,
  ...
  update(id: string, patch: Partial<FoodMenuItem>): void {
    const existing = this.items$.value.find((item) => item.id === id);

    if (!existing) {
      return;
    }

    this.api.update<FoodMenuItem>('food-menu', id, {
      ...existing,
      ...patch,
